...
## 7. 当前包是否支持 O 兼容和 M 兼容？

当前包不是 O/M 专用方言。

真实环境探针显示，当前连接库 `datcompatibility=A`，可以执行部分 Oracle 风格语法，例如 `dual`、`nvl`、`sysdate`、`rownum`；但 MySQL 风格语法如反引号、`ifnull()`、`auto_increment` 不通过。

因此当前结论是：

- PG 基础路径：已支持并验证
- A/O 兼容库中的部分 Oracle 风格 SQL：可通过当前协议路径执行，但不等于完整 O 方言支持
- M/MySQL 兼容：当前包不支持，应单独走 MySQL 协议和 MySQL SQLAlchemy 方言路线验证
